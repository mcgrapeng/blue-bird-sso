...
# 基础
export JAVA_HOME=/bird/tools/jdk8
export PATH=PATH=$PATH:$JAVA_HOME/bin
export CLASSPATH=$JAVA_HOME/jre/lib/rt.jar:$JAVA_HOME/lib/dt.jar:$JAVA_HOME/lib/tools.jar

DATE=$(date +%Y%m%d%H%M)
# 基础路径
BASE_PATH=/bird/deploy/dev/sso
# 编译后 jar 的地址。部署时，Jenkins 会上传 jar 包到该目录下
SOURCE_PATH=$BASE_PATH/build
# 服务名称。同时约定部署服务的 jar 包名字也为它。
SERVER_NAME=bird-common-sso
# 环境
PROFILES_ACTIVE=dev
# 健康检查 URL
#HEALTH_CHECK_URL=http://127.0.0.1:8090/actuator/health/
...
function start() {
    ...
    # 开始启动
    BUILD_ID=dontKillMe nohup java -server $JAVA_OPS -jar $BASE_PATH/$SERVER_NAME.jar --spring.profiles.active=$PROFILES_ACTIVE >/dev/null 2>&1 &
    
    sleep 10s 

#BUILD_ID=dontKillMe nohup java -server $JAVA_OPS $JAVA_AGENT -jar $BASE_PATH/$SERVER_NAME.jar --spring.profiles.active=$PROFILES_ACTIVE >> /bird/deploy#/dev/sso/logs/sso.log 2>&1 &
